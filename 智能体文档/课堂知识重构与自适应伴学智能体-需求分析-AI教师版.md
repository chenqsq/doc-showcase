...
- 一期平台模式固定为 `Standard`。
- `Multi-Agent` 仅作为 `P2` 扩展，不进入一期必交付清单。
...
| --- | --- | --- | --- | --- | --- |
| FR-01 | SCN-01 | 策略加载 | 应用配置/提示词 | AC-01 | 第 1 步 |
| FR-02 | SCN-01/02 | 多模态解析 | 文档解析/OCR/ASR | AC-02, AC-03 | 第 1-2 步 |
| FR-03 | SCN-02 | 学习诊断 | 大模型+知识检索 | AC-04 | 第 2 步 |
| FR-04 | SCN-02 | 分层讲解 | RAG+回复节点 | AC-05, AC-06 | 第 3 步 |
| FR-05 | SCN-03 | 练习生成 | 工作流大模型节点 | AC-07 | 第 4 步 |
| FR-06 | SCN-03 | 测评评分 | 条件节点+评分逻辑 | AC-08 | 第 4 步 |
| FR-07 | SCN-03 | 错因归因 | 标签提取+知识检索 | AC-09 | 第 5 步 |
| FR-08 | SCN-04 | 学习计划输出 | 总结节点+记忆写回 | AC-10 | 第 5 步 |
| FR-09 | SCN-04 | 看板聚合 | 分析能力/API | AC-11 | 扩展演示 |
| FR-10 | SCN-02/04 | 检索控制 | custom_variables + 标签映射 | AC-12 | 扩展演示 |
| FR-11 | 全场景 | 回归评测 | 基准评测/对比评测 | AC-13 | 评测页 |
| FR-12 | 全场景 | 发布接入 | 发布/WebSocket/SSE | AC-14 | 发布页 |
...
```mermaid
flowchart TD
    A["P0: AI教师核心闭环"] --> B["P1: 精准化与运营化"]
    B --> C["P2: 扩展接入与协作化"]

    A1["FR-01~FR-08"] --> A
    B1["FR-09~FR-11"] --> B
    C1["FR-12 + Multi-Agent扩展"] --> C
```
...
| 阶段 | 交付内容 | 出口标准 |
| --- | --- | --- |
| P0 | FR-01~FR-08 | 完整跑通“讲解-练习-测评-复盘” |
| P1 | FR-09~FR-11 | 运营看板与评测体系可复现 |
| P2 | FR-12 + 扩展能力 | 发布接入稳定，协作能力可演示 |
...
2. 平台优先：一期坚持 ADP `Standard` 模式，稳定集成知识库、工作流、插件/API。
3. 验收优先：每个 P0 功能均有 AC 对应和演示落点。
