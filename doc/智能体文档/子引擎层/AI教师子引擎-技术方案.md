...
`子引擎能力面` 的正式定义见 [AI主导学习平台-总体架构设计.md](../平台层/AI主导学习平台-总体架构设计.md)。  
本文只负责把这个上游定义落成可调用、可回流、可被平台接住的技术接口。
...
`子引擎回流结果` 的字段级正式定义以本文为准。  
`AI教师子引擎-PRD` 只负责提出必须有结构化回流结果的要求，实施附录和学科配置文档只负责承接这里的字段。
...
`会话与过程记录` 的正式定义见 [AI主导学习平台-学习生命周期与编排策略.md](../平台层/AI主导学习平台-学习生命周期与编排策略.md)。  
本文只说明子引擎技术接口如何承接这些上游过程记录，而不重复定义它们。
