...
下面这些骨架词沿用上游正式定义。  
本文只负责把它们翻译成验收时应该看到什么，而不重新定义它们。

| 骨架词 | 平台验收时要看到什么 |
| --- | --- |
| `平台动作面` | 建档、目录树、当前任务卡、自动推进、双层笔记、教师运营入口、学科接入都能被单独解释和演示 |
| `会话与过程记录` | 学习会话、推进日志、课节笔记、总复习本之间有清楚对象链路 |
| `轻量路由与启动装配` | 学生进入平台后能沿着固定链路被带进本节学习，而不是临时拼接流程 |
| `权限边界与占位结构` | 平台、子引擎、学科层职责不混乱，P1/P2 增强项和扩科槽位有明确边界 |
| `子引擎能力面` | 平台验收不要求子引擎接管平台编排，但要求它稳定输出平台能接住的结构化结果 |
